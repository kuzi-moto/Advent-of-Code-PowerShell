# Grabbed this code from https://github.com/dfinke/PowerShellCombinations
class Permutation {
  hidden [int[]] $data
  hidden [int] $order
  ...
  [int[]] ApplyTo([string[]] $sa) {
    if ($sa.Length -ne $this.order) { throw "Bad array size in ApplyTo()" }

    [int[]]$result = New-Object string[] $this.order
