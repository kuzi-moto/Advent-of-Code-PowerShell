...
function Move-Crate {
  ...
  # Gets the crate to move
  $Crate = $Layout[$Source][-1]

  # Remove from source
  $Layout[$Source].RemoveAt($Layout[$Source].Count - 1)

  # Add to destination
  $null = $Layout[$Destination].add($Crate)

  $Count--

  if ($Count -gt 0) {
    Move-Crate $Layout $Count $Source $Destination
  }
  else {
    $Layout
  }
...
for ($i = $StackIndex + 2; $i -lt $Data.Count; $i++) {
  ...
  Write-Host
  $Data[$i]
  $CrateLayout = Move-Crate $CrateLayout $Matches[1] ($Matches[2] - 1) ($Matches[3] - 1)
